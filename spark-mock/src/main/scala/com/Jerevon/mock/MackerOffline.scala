package com.Jerevon.mock

import java.text.SimpleDateFormat
import java.util.UUID

import com.Jerevon.datamodel.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import com.Jerevon.randomUtils.{RanOpt, RandomDate, RandomNum, RandomOptions}
import com.Jerevon.util.ConfigurationUtil
...
object MockerOffline {
  ...
  val searchKeywordsOptions = RandomOptions(RanOpt("手机", 30), RanOpt("笔记本", 70), RanOpt("内存", 70), RanOpt("i7", 70),
    RanOpt("苹果", 70), RanOpt("吃鸡", 70))
  val actionsOptions = RandomOptions(RanOpt("search", 20), RanOpt("click", 60), RanOpt("order", 6), RanOpt("pay", 4),
    RanOpt("quit", 10))
